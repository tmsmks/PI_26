...
15. [Data leakage — identification et correction](#15-data-leakage--identification-et-correction)
...
## 10. Calcul de la prédiction prévisionnelle J+7 (onglet 2)
...
## 15. Data leakage — identification et correction

### Le problème initial

La première version du modèle (mono-Lacor) atteignait des scores >99 %
suspects. L'analyse a révélé du **data leakage** : certaines features
contenaient directement l'information de la cible.
...
| `generators_kw` | Le générateur s'allume **pendant** la coupure | Conséquence |
| `generator_active` | Idem | Conséquence |
| `generator_ratio` | Idem | Conséquence |

### Correction

Ces 7 features + `datetime` et `is_outage` (cible) + 4 colonnes
constantes/redondantes sont exclues via `COLS_TO_DROP` dans
`train_baseline.py` (voir [section 3.1](#31-sélection-des-features-cols_to_drop)).

### Features d'historique conservées

Les features `hours_since_last_outage`, `last_outage_duration_h`,
`outage_frequency_7d`, `avg_outage_duration_7d`, `outage_trend_7d` **sont
conservées** car elles sont calculées avec un `shift(1)` par hôpital sur
`is_outage` (`add_outage_history_features` dans `build_features.py`) :
elles ne voient donc que le passé strict de la série.

C'est d'ailleurs `hours_since_last_outage` qui domine désormais
...
| Amélioration | Statut |
|--------------|--------|
| Comparaison multi-modèles (RF / XGBoost / LightGBM) | ✅ |
...
| Gestion d'erreurs dans l'app Streamlit | ✅ |
